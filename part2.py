...
class NetworkLstm(tnn.Module):
    ...
    def __init__(self):
        ...
        """
        self._lstm = tnn.LSTM(50, 100, batch_first=True)
    ...
    def forward(self, input, length):
        ...
        out = torch.nn.functional.relu(self._l1(out.contiguous().view(-1, 100)))
        out = self._l2(out.contiguous().view(-1, 64))
...
class NetworkCnn(tnn.Module):
    ...
    def __init__(self):
        ...
        Create and initialise weights and biases for the layers.
        """
        self._conv = tnn.Conv1d(in_channels=50, out_channels=50, kernel_size=8, padding=5)
        self._pool = tnn.MaxPool1d(kernel_size=4)
        self._l1 = tnn.Linear(50,1)
    ...
    def forward(self, input, length):
        ...
        Create the forward pass through the network.
        """
        out = torch.nn.functional.relu(self._conv(input.permute(0, 2, 1)))
        out = self._pool(out)
        out = torch.nn.functional.relu(self._conv(out))
        out = self._pool(out)
        out = torch.nn.functional.relu(self._conv(out))
        out = self._global(out)
        out = self._l1(out.view(-1,50))
        out = out.view(len(length), -1)
        return out[:, -1]
...
def measures(outputs, labels):
    ...
    for o, l in zip(outputs, labels):
        outBool = o.item() > 0
        labelBool = l.item() == 1 
        if outBool and labelBool:
            tp += 1
        elif not outBool and labelBool:
            fn += 1
        elif outBool and not labelBool:
            fp += 1
        else:
            tn += 1
    return tp, tn, fp, fn
